...
// Simplified workspace button components (inspired by reference)
function WorkspaceButton({ state, number, onClick }: { 
    state: 'focused' | 'active' | 'occupied' | 'empty', 
    number?: number,
    onClick: () => void 
}) {
    return (
        <button className={`workspace ${state}`} onClick={onClick}>
            <box className={`workspace-button ${state}`}>
                {state === 'empty' && number && (
                    <label className="workspace-number">{number}</label>
                )}
            </box>
        </button>
    )
...
function LeftSection() {
    return (
        <box className="bar-section bar-left" halign={Gtk.Align.START}>
